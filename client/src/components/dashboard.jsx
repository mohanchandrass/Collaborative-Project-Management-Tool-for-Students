  ...
  const exportBacklogToExcel = async () => {
    try {
          ...
          await Promise.all(
            groupData.members.map(async (memberId) => {
              ...
              if (userSnap.exists()) {
                ...
                memberMap[memberId] = userData.displayName || userData.username || userData.email || 'Unnamed';
              }
            })
          );
        }
      ...
      Object.keys(groupedTasks).forEach(projectName => {
        const worksheet = XLSX.utils.json_to_sheet(groupedTasks[projectName]);
      ...
      <div className="dashboard-grid">
        <section className="dashboard-card instructions-card">
          ...
          <ol className="instruction-list">
            <li className="instruction-item"><span className="instruction-step">1</span>
              <div>
                <h3>Create a New Project</h3>
                <p>Fill out the project details form with name, description, and deadline.</p>
              </div>
            </li>
            <li className="instruction-item"><span className="instruction-step">2</span>
              <div>
                <h3>Add Tasks</h3>
                <p>Specify task details and assign them to team members.</p>
              </div>
            </li>
            <li className="instruction-item"><span className="instruction-step">3</span>
              <div>
                <h3>View Projects</h3>
                <p>Navigate to the Projects section to view and edit your projects.</p>
              </div>
            </li>
            <li className="instruction-item"><span className="instruction-step">4</span>
              <div>
                <h3>Monitor Progress</h3>
                <p>Track task status (To Do, In Progress, Completed).</p>
              </div>
            </li>
            <li className="instruction-item"><span className="instruction-step">5</span>
              <div>
        ...
        <section className="dashboard-card quick-access-card">
          ...
          <div className="tools-grid">
            <button className="tool-button" onClick={() => setActiveTab('my-projects')}>
              <div className="tool-icon"><FaFolder size={24} /></div>
              <span>All Projects</span>
            </button>

            <button className="tool-button" onClick={() => setActiveTab('group-chat')}>
              <div className="tool-icon"><FaUsers size={24} /></div>
              <span>Group Chat</span>
            </button>

            <button className="tool-button" onClick={() => setActiveTab('task-status')}>
              <div className="tool-icon"><FaCheckSquare size={24} /></div>
              <span>Task Status</span>
            </button>

            <button className="tool-button" onClick={exportBacklogToExcel}>
              <div className="tool-icon"><FaTasks size={24} /></div>
              <span>Generate Product Backlog</span>
            </button>

            <Link to="/report-generator" className="tool-button">
              <div className="tool-icon"><FaFileAlt size={24} /></div>
