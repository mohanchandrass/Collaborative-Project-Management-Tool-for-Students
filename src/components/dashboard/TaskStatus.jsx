...
const TaskStatus = ({ groupId }) => {  // Pass groupId as a prop
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(
      query(
        ...
        where('groupId', '==', groupId)  // Filter tasks by groupId
      ),
      (querySnapshot) => {
        const loadedTasks = [];
  ...
  const handleAddTask = async (e) => {
    ...
    try {
      await addDoc(collection(firestore, 'groups', groupId, 'tasks'), {
        ...
        groupId: groupId,  // Add groupId to the task
      });
      setNewTaskName('');
    ...
    <div className="task-status-container">
      ...
      <h2 style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div className='view-chart-button'>
          <a href="/chart">
            <button style={{ marginLeft: '1rem' }}>View Chart</button>
          </a>
        </div>
      </h2>
      ...
      {/* List of Tasks with Edit/Delete */}
      <h3>Manage Tasks</h3>
